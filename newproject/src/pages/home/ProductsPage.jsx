    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="container-fluid px-2 px-md-4 px-lg-5 my-4">
            ...
            {categoriesList.map(cat => (
              <button
                ...
                onClick={() => {
                  ...
                  if (seasonParam) newParams.season = seasonParam;
                ...
                style={{
                  ...
                  backgroundColor: categoryParam === cat ? '#20b2aa' : 'white',
                  color: categoryParam === cat ? 'white' : '#20b2aa',
          ...
          {/* ── HEADING ── */}
          <h1 className="mb-4">
            {seasonParam && <span className="text-capitalize">{seasonParam} Collection - </span>}
          ...
          <div className="row g-2 g-md-4">
            {allProducts.length === 0 ? (
              <p className="text-muted">
                ...
                {categoryParam !== 'All' && <> in <strong>{categoryParam}</strong></>}
                {seasonParam && <> in <strong>{seasonParam} Collection</strong></>}
              </p>
            ) : (
              allProducts.map((item) => (
                <div className="col-6 col-md-4 col-lg-3 mb-5 position-relative" key={item.id}>
                  ...
                  <div className="position-absolute top-0 end-0 ms-5 translate-middle-x d-flex">
                    <button
                      className={`btn ${favorites.some(f => f.id === item.id) ? "btn-danger" : "btn-outline-danger"}`}
                      onClick={(e) => toggleFavorite(e, item)}
                    >
                      <i className={`bi ${favorites.some(f => f.id === item.id) ? "bi-heart-fill" : "bi-heart"}`} />
                    </button>
                  </div>
                </div>
              ))
